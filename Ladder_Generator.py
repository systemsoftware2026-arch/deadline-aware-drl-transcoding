...
MODEL_PATHS = {
    "PPO_DRL": "model/default/default2.zip",
    "PPO_BASELINE": "model/default/pred_base.zip"
}
...
if __name__ == "__main__":
    ...
    def export_plan(env, policy_name="DRL", alloc=None, out_dir="plans", suffix=None):
        """
        ...
        filename = f"{policy_name}_{suffix}.json" if suffix else f"{policy_name}.json"
    ...
    if TESTING:
        ...
        for SIZE_FACTOR in tqdm(np.arange(0.01, 0.99, 0.01), desc="SIZE FACTOR Progress"):
        ...
        TARGET_THRESHOLDS = ["PR_90", "PR_95"]

        for algo, results in pr.items():
            for th in TARGET_THRESHOLDS:
                size_factor = results.get(th, None)
                if size_factor is None:
                    print(f"[WARN] {algo} has no PR value for {th}, skipping...")
                    continue

                print(f"\n===== Building ladder for {algo} at {th} = {size_factor} =====")

                
                SIZE_FACTOR = size_factor   # 🔥 PR 기반으로 글로벌 값 업데이트
                ...
                # ---------------- DRL / BASELINE DRL rollout ---------------- #
                if algo in ["PPO_DRL", "PPO_BASELINE"]:
                    model = MaskablePPO.load(MODEL_PATHS[algo], env=test_env, device=device)
                    ...
                    export_plan(env, algo, None, "plans", suffix=th)

                # ---------------- MCKP greedy case ---------------- #
                elif algo == "MCKP_predict":

                # 🔥 reset 후 greedy 한번 실행해서 alloc 생성
                    env.MCKP_predict_value, env.MCKP_predict_alloc = env.greedy_allocation_combo_delta(mode="mckp")

                    # 🔥 export at required PR value
                    export_plan(env, algo, env.MCKP_predict_alloc, "plans", suffix=th)

                else:
                    print(f"[WARN] {algo} is not included in TTA test set, skipping...")
                    continue
